...
from image_sharing import ETCD_HOST_PORT, ETCD_HOST_URL
from image_sharing.database import get_db
...
def check_liveness(request: Request) -> Response:
    data = {
        'status': 'pass',
        'checks': {'etcd:status': [check_etcd_connection()], 'postgres:status': [check_etcd_connection()]},
    }
